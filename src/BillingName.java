...
public class BillingName {
    ...
    public void SelectBusinessDetailsTable(int BusinessID){
        String sqlStmt = null;
        ResultSet rs = null;
        //try{
            sqlStmt = "select BID,Name,Alias,Address,City,Province,PostalCode,ContactName," 
                + "PrimaryPhone,Ext,SecondaryPhone,Ext2,Fax,EmailAddress " 
                + "FROM Business_Details where BID = '" + BusinessID + "';";
                //tblName = "Business_Details";
            rs = SQLConnection.getRecordSet(sqlStmt);
            BillingName updt = new BillingName(rs);
        //}
        //catch(SQLException ex){
            
        //}
    }
    public void InsertBusinessDetailsTable(){
        String sqlStmt = null;
        try{
            sqlStmt = "INSERT INTO Business_Details "
                    + "(Name,Alias,Address,City,Province,PostalCode,ContactName,PrimaryPhone,Ext,SecondaryPhone,Ext2,Fax,EmailAddress,UpdateDate,User_id) "
                    + "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            System.out.println(sqlStmt);
            PreparedStatement stmt = SQLConnection.conn.prepareStatement(sqlStmt);
            //Integer ServiceReqNr generated by the db
            stmt.setString(1,this.BillingName );
            stmt.setString(2,this.Alias );
            stmt.setString(3,this.Address);
            stmt.setString(4,this.City );
            stmt.setString(5,this.Province);
            stmt.setString(6,this.PostalCode);
            stmt.setString(7,this.ContactName);
            stmt.setString(8,this.Phone1);
            stmt.setString(9,this.Ext);
            stmt.setString(10,this.Phone2);
            stmt.setString(10,this.Fax);
            stmt.setString(11,this.Email);
            stmt.setString(12,DateUtils.now_date_time());
            //stmt.setInt(13,this.); //user_id        
            stmt.executeUpdate();   
        }
    ...
    public void UpdateBusinessDetailsTable(){
        String sqlStmt = null;
        sqlStmt = "UPDATE Business_Details SET Name = '" + this.BillingName + "'," 
            + " Alias = '" + this.Alias + "',"
